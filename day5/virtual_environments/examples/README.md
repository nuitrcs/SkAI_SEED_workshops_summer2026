...
*Challenge* : Add your frequently used environments, in this case say the two envs created above, as ipykernels so that they can be selected from Jupyter's IDE directly.
    ...
    ```
Then go back to Jupyter, change the Python kernel and try running `test_numpy1.ipynb` and `test_numpy2.ipynb` notebooks, now using the Jupyter IDE directly to change the working environment.